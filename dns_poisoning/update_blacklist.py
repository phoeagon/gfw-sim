...
def combine_lists(content, user_rule=None):
    builtin_rules = open('resources/builtin.txt', 'r').readlines()
...
def reduce_domains(domains):
    ...
    tlds = set([x.strip() for x in open('resources/tld.txt', 'r').readlines()])
    new_domains = set()
...
def generate_pac_fast(domains):
    proxy_content = "\n".join([x.replace("\n", '')
                               for x in list(domains)])
